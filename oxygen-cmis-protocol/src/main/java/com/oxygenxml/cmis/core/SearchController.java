...
public class SearchController {
	...
	private List<IResource> queryResourceName(String name, int searchObjectTypes) {
		...
		ItemIterable<CmisObject> results = ctrl.getSession().queryObjects(scope, "cmis:name LIKE '%" + name + "%'",
				false, oc);
	...
	private List<IResource> queryResource(String content, int searchObjectTypes) {
		...
		ItemIterable<CmisObject> results = ctrl.getSession().queryObjects(scope, "cmis:name LIKE '%" + content
				+ "%' OR cmis:description LIKE '%" + content + "%' OR cmis:createdBy LIKE '%" + content + "%'", false,
				oc);
	...
	public List<IDocument> queryDocContent(String content) {
		...
		OperationContext oc = ctrl.getSession().createOperationContext();
		// oc.setFilterString("cmis:objectId,cmis:name,cmis:createdBy");
		ItemIterable<CmisObject> results = ctrl.getSession().queryObjects("cmis:document",
				"CONTAINS ('" + content + "')", false, oc);
